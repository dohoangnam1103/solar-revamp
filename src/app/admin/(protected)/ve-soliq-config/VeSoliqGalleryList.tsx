...
import { deleteVeSoliqGalleryImage, reorderVeSoliqGalleryImages } from '@/app/actions/admin-crud'
...
function moveItem(items: GalleryImage[], fromId: number, toId: number) {
...
export default function VeSoliqGalleryList({ images }: Props) {
  ...
  const persistOrder = (nextItems: GalleryImage[]) => {
    startTransition(async () => {
      await reorderVeSoliqGalleryImages(nextItems.map((item) => item.id))
      router.refresh()
    })
  }
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-gray-300 bg-white px-4 py-8 text-center text-sm text-gray-400">
        Chưa upload ảnh nào — đang dùng bộ ảnh mặc định.
      </div>
    )
  }
        ...
        {items.map((image, index) => (
          ...
          >
            ...
            <div className="flex items-center justify-end gap-2">
              <form action={deleteVeSoliqGalleryImage.bind(null, image.id)}>
